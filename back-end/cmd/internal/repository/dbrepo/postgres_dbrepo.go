...
const dbTimeout = time.Second * 3 // users can dissaper unexpected, lost connection, close laptop....
...
func (m *PostgresDBRepo) OneMovie(id int) (*models.Movie, error) {
	...
	err := row.Scan( // scan data from database inside my variable
		&movie.ID,
		&movie.Title,
		&movie.ReleseDate,
		&movie.RunTime,
	...
	// it is ok not to have genre assosiated with this movie
	if err != nil && err != sql.ErrNoRows {
		return nil, err
	}
	defer rows.Close() // before this function exsits close this connection to db

	var genres []*models.Genre
	for rows.Next() {
		var g models.Genre //declare
		err := rows.Scan(
			&g.ID,
			&g.Genre,
		)
...
func (m *PostgresDBRepo) OneMovieForEdit(id int) (*models.Movie, []*models.Genre, error) {
	...
	var genres []*models.Genre // looping trough genres assosiated with this movie
	var genresArray []int
...
func (m *PostgresDBRepo) DeleteMovie(id int) error {
	// we dont need to delete genres, because there is foregin key in database, it will be delete automatically
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
